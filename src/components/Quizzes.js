...
const Quizzes = ({ navigate }) => {
    ...
    try {
      ...
      unsubscribes.push(onSnapshot(userResultsQuery, (snapshot) => {
        ...
        snapshot.docs.forEach(doc => {
          ...
          fetchedResultsMap[resultData.quizId] = { id: doc.id, ...resultData };
        });
  ...
  }, [db, currentUser, loadingAuth, navigate, appId]);
      ...
      <div className="flex-grow p-6 md:p-8 bg-white rounded-xl shadow-xl ml-4 mr-4 mt-4 mb-4">
        ...
        {loadingData ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {quizzes.map(quiz => {
              const hasTakenQuiz = userQuizResults[quiz.id];
              return (
                <div key={quiz.id} className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                  ...
                  {hasTakenQuiz ? (
                    ...
                    <button
                      onClick={() => navigate('quizTaking', { quizId: quiz.id })} // ✅ Corrected here
